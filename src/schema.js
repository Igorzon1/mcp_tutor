...
const common = {
  title: shortText,
  stage: STAGE_ORDER.default('predict'),
};
...
const codeBlockSchema = z.object({
  ...
  type: z.literal('code'),
  prompt: text,
  language: z.enum(['html', 'javascript']).default('html'),
  starterCode: z.string().max(40000).default(''),
  checks: z.array(checkSchema).max(20).default([]),
  tests: z.array(javascriptTestSchema).max(20).default([]),
...
export const TOOL_DEFINITIONS = [
  ...
  { name: 'tutor_add_block', description: 'Adiciona conteúdo ao painel: message, quiz, code (HTML/CSS ou JavaScript), reflection, diagram ou chart. Informe a etapa atual em stage. Em code JavaScript, use tests com expressões booleanas para verificar a ideia praticada. Só uma atividade respondível (quiz, code ou reflection) pode ficar aberta por vez; adapte à tentativa anterior e não forneça a solução antes de o aluno tentar.', schema: z.object({ sessionId: z.string().uuid(), block: blockSchema }).strict() },
...
Código HTML/CSS é visualizado sem JavaScript e sem rede. JavaScript só é executado no Web Worker descartável do painel, com limite de tempo, sem APIs de rede, DOM ou arquivos; trate o resultado como evidência para revisão, não como nota automática. Nunca execute código do aluno no terminal. Outras linguagens podem ser discutidas em reflection, mas ainda não têm executor neste MVP.`;
