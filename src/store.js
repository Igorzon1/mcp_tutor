...
import { reviewCardSchema, recallSchema, ratingSchema, isLocalCode } from './schema.js';
...
function migrateSession(session) {
  ...
  const parsed = sessionSchema.safeParse({ title: session.title, goal: session.goal, level: session.level, objectives: session.objectives, timeBudgetMinutes: session.timeBudgetMinutes, ...(session.learnerProfile ? { learnerProfile: session.learnerProfile } : {}) });
  ...
  const completedTransfer = session.mode === 'demo' && session.blocks.find(block => block.demoStep === 'transfer' && session.advanced.includes(block.id));
  if (completedTransfer) normalized.progress = { stage: 'transfer', status: 'completed', activeBlockId: null, lastAttemptId: latestAttemptFor(session, completedTransfer.id)?.id || null, completedStages: [...STAGES] };
  return normalized;
}
...
export class Store {
  ...
  async init() {
    ...
    try {
      ...
      const migrationNeeded = data.version !== DATA_VERSION || data.sessions.some(session => !Array.isArray(session.reviews) || !session.learnerProfile || !Object.hasOwn(session, 'plan') || !validProgress(session.progress));
  ...
  append(session, rawBlock, changed, extra = {}) {
    ...
    if (block.stage !== progress.stage && !(block.type === 'lesson' && block.stage === 'learn') && !(block.type === 'message' && block.stage === 'review')) throw new AppError(`A etapa atual é ${progress.stage}. Crie o conteúdo com stage igual à etapa atual.`);
  ...
  async create(raw, isDemo = false, demoLanguage = 'html') {
    ...
    return this.mutate((data, changed) => {
      ...
      const session = { ...input, learnerProfile: input.learnerProfile || initialProfile(), plan: null, id: randomUUID(), mode: isDemo ? 'demo' : 'tutor', createdAt: now(), updatedAt: now(), revision: 0, progress: initialProgress(), demoLanguage: isDemo ? demoLanguage : undefined, blocks: [], attempts: [], events: [], advanced: [], reviews: [] };
  ...
  list() {
    return this.data.sessions.map(session => {
      ...
      return { id: session.id, title: session.title, goal: session.goal, level: session.level, mode: session.mode, updatedAt: session.updatedAt, revision: session.revision, progress: copy(progress), hasPlan: !!session.plan, activityCount: session.blocks.filter(block => block.type !== 'message').length, messageCount: session.blocks.filter(block => block.type === 'message').length, attemptCount: session.attempts.length, summary: learningSummary(session) };
  ...
  advanceDemo(session, block, result, changed) {
    ...
    if (block.demoStep === 'prediction') {
      ...
      this.append(session, pack.practice, changed, { demoStep: 'practice' });
    }
    if (block.demoStep === 'practice') {
      this.transitionStage(session, changed, session.progress.lastAttemptId, 'A solução passou nos casos propostos.');
    ...
    if (block.demoStep === 'reflection') {
      ...
      this.append(session, pack.transfer, changed, { demoStep: 'transfer' });
    }
    if (block.demoStep === 'transfer') {
      this.transitionStage(session, changed, session.progress.lastAttemptId, 'A nova aplicação passou nos casos propostos.');
  ...
  async submit(id, raw, { signal } = {}) {
    ...
    const originalBlock = original.blocks.find(block => block.id === input.blockId);
    if (!originalBlock || !isAnswerable(originalBlock)) throw new AppError('Atividade não aceita respostas.');
    if (original.progress.activeBlockId !== originalBlock.id || original.progress.status !== 'awaiting_attempt') throw new AppError('Esta não é a atividade respondível atual ou está aguardando avaliação do tutor.');
