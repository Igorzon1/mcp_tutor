...
function renderConversationOnboarding() {
  $('#conversation-onboarding').hidden = !session || session.blocks.some(block => block.type === 'message') || (chatPending && chatPendingSessionId === session.id);
}
...
function renderChallengeRail() {
  ...
  const kind = challenge.type === 'code'
    ? (challenge.language === 'javascript' ? 'JavaScript' : 'HTML/CSS')
  ...
  $('#challenge-prompt').innerHTML = formatMessage(challenge.prompt || challenge.caption);
  const checks = challenge.language === 'javascript' ? challenge.tests : challenge.checks;
  const labels = checks?.length ? checks.map(check => check.label) : challenge.type === 'quiz' ? [`${challenge.options?.length || 0} opções para escolher`] : [];
...
function renderPlanRail() {
  ...
  const outcomes = Array.isArray(plan?.outcomes) ? plan.outcomes.map(planValue).filter(Boolean) : [];
...
function renderSession() {
  ...
  $('#lesson-mode').textContent = session.mode === 'demo' ? 'EXEMPLO GUIADO · SEM IA' : 'SUA CONVERSA DE APRENDIZAGEM';
  ...
  const challenge = currentChallenge();
  const messageBlocks = session.blocks.filter(block => block.type === 'message');
  ...
  if (messageBlocks.length > renderedMessageCount) {
    if (nearEnd || forceChatScroll || !renderedMessageCount) requestAnimationFrame(scrollToLatest);
...
function createForm(block, container) {
  ...
  if (block.type === 'code') {
    const javascript = block.language === 'javascript';
    const fileName = javascript ? 'lesson.js' : 'index.html';
    ...
    const action = javascript ? '<button type="button" class="run-code">▷ &nbsp; Executar testes</button>' : '<button type="button" class="run-preview">▷ &nbsp; Visualizar</button>';
    const result = javascript ? '<div class="runtime-output" hidden><div class="runtime-output-heading"><span>RESULTADO DOS TESTES</span><span class="runtime-status"></span></div><div class="runtime-checks"></div><pre class="runtime-result"></pre></div>' : '<div class="preview" hidden><div class="preview-bar"><span>SUA PÁGINA</span><span>HTML + CSS · sem JavaScript ou rede</span></div><iframe title="Prévia da sua página HTML" sandbox="" referrerpolicy="no-referrer"></iframe></div>';
    fields = `${requirements}<div class="editor-shell"><div class="editor-bar"><span class="editor-file"><span aria-hidden="true">◇</span> ${fileName}</span>${action}</div><div class="editor-body"><pre class="line-numbers" aria-hidden="true"></pre><textarea class="code-editor" name="answer" aria-label="Código ${javascript ? 'JavaScript' : 'HTML'} da atividade ${escape(block.title)}" spellcheck="false" autocomplete="off" autocapitalize="off" maxlength="50000" required>${escape(draft.answer ?? block.starterCode)}</textarea></div></div>${result}`;
  }
  ...
  if (block.type === 'code') {
    ...
    installCodeEditor(editor, { language: block.language });
    const help = document.createElement('p');
    help.className = 'editor-help';
    help.id = `editor-help-${block.id}`;
    help.textContent = block.language === 'javascript'
  ...
  form.onsubmit = async event => {
    ...
    finally { delete form.dataset.submitting; if (form.isConnected) updateBlock(block, form.closest('.activity')); }
  };
}
function updateBlock(block, element) {
  ...
  if (form) form.querySelectorAll('input, textarea, select, button').forEach(control => { control.disabled = !active || !!form.dataset.submitting; });
