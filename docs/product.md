...
**Versão documentada: 0.4.** O escopo permanece em 0.4 nesta revisão; não há incremento de versão.
...
- a execução JavaScript é útil para exercícios pequenos, mas não é uma plataforma de execução segura para código hostil, aplicações completas ou outras linguagens;
