# Arquitetura e fronteiras de confiança — versão 0.5
...
Codex / Claude Code / outro cliente MCP
        ...
        src/mcp.js
         ...
         src/server.js ───────► public/
               ...
               │                     ├─ `/exercise-worker.js` para JS com testes booleanos
               │                     └─ Laboratório e revisões
               ├─ src/runner.js → Bubblewrap + seccomp → runtimes locais
              ▼
...
## Execução local e revisão na v0.5

O serviço compartilha um CodeRunner com o Store. Python, Java, C, C++ e Node (runtime local) usam namespaces do Bubblewrap, /usr somente para leitura, pastas temporárias limitadas, ambiente limpo e libseccomp. O programa não recebe pastas pessoais, credenciais ou rede. A compilação e a execução compartilham o tempo limite; a API aceita até duas execuções simultâneas. Limites e ausência de fallback sem isolamento estão no README. Não é uma VM ou garantia contra falhas do kernel/runtime.

JavaScript antigo mantém Worker e evidência pending_review. Resultados locais são produzidos no servidor e comparados com casos definidos, mas testes verdes não comprovam domínio. O controle de uma atividade respondível por vez e os avanços explícitos do tutor permanecem. As avaliações nativas rodam fora da fila de persistência e o estado da atividade é revalidado antes de salvar.

Revisões ficam em session.reviews e têm perguntas, referência, tentativas e autoavaliação. A API pública oculta a referência até uma tentativa ainda não avaliada. O agendamento é independente das quatro etapas de progress; lesson/learn e message/review são conteúdos de apoio. O arquivo continua na versão de dados 3 e preserva campos de sessões antigas.
